...
<html  lang="ar"  dir="rtl">
    ...
    <?php
    $OrderValue = 0;
    $localhost20 = "localhost";
    $dbconnection20 = "root";
    $dbpassword20 = "rootroot";
    $dbname20 = "glomus";
    $conn20 = new mysqli($localhost20, $dbconnection20, $dbpassword20, $dbname20);
    if ($conn20->connect_error) {
        die("Connection failed: " . $conn20->connect_error);
    }
    $sql20 = "call GetLastOrderId()";
    $sql20 = "call GetLastOrderId()";
    $results20 = mysqli_query($conn20, $sql20);
    $row20 = mysqli_fetch_assoc($results20);


    $localhost30 = "localhost";
    $dbconnection30 = "root";
    $dbpassword30 = "rootroot";
    $dbname30 = "glomus";
    $conn30 = new mysqli($localhost30, $dbconnection30, $dbpassword30, $dbname30);
    if ($conn30->connect_error) {
        die("Connection failed: " . $conn30->connect_error);
    }
    $sql30 = "select * from OrderDescription where OrderMainID = '" . $row20['OrderID'] . "'";
    $results30 = mysqli_query($conn30, $sql30);
    while ($row30 = mysqli_fetch_assoc($results30)) {
        if ($row30['ProDest'] == 1) {
            $OrderValue = 1;
        }
    }
    ?>
    <body onafterprint="AfterPrint(<?php echo $OrderValue; ?>)" onload="PrintLoad()">
    ...
    <!--            <img src="Images/Logo1.jpg" ALIGN="right">-->
            ...
            <?php if ($row['OrderType'] == 1): ?>
                ...
                <hr class="bottom">
                <?php
                $sql2 = "call SelectFromOrderDescription('" . $row['OrderID'] . "')";
                $results2 = mysqli_query($conn2, $sql2);
                ?>
                <?php while ($row2 = mysqli_fetch_assoc($results2)): ?>
                    <div class="shipping-1">
                        <p><?php echo $row2['ProName']; ?></p>
                        <p><?php echo $row2['ProCost']; ?></p>
                        <p><?php echo $row2['ProQty']; ?></p>
                        <p><?php echo $row2['ProTotalCost']; ?></p>
                    </div>
                <?php endwhile; ?>
                <hr class="top">
                <div class="shipping-total-2">
                    <h6>الـمـبـلـغ</h6>
                    <h6 style="font-weight: bold;"><?php echo ($row['OrderCost'] + $row['OrderDiscountCost']); ?></h6>
                </div> 
                <div class="shipping-total-2">
                    <h6>خـصـم</h6>
                    <h6 style="font-weight: bold;"><?php echo $row['OrderDiscountCost']; ?></h6>
                </div> 
                <div class="shipping-total-2">
                    <h6>الـمـبـلـغ الـكـلـي</h6>
                    <h6 style="font-weight: bold;"><?php echo $row['OrderCost']; ?></h6>
                </div>
            <?php else : ?>
                <div class="shipping-info-head">
                    <h6>رقـم الـفـاتـورة # : <?php echo $row['OrderID']; ?></h6>
                ...
                <div class="shipping-info-head">
                    <h6> رقم التليفون : </h6>
                    <h6>  <?php echo $row['OrderPhone']; ?>  </h6>

                </div>
                <div class="shipping-info-head">
                    <h6>  الأســم :</h6>
                    <h6>  <?php echo $row['OrderName']; ?>  </h6>
                </div>
                <div class="shipping-info-head">
                    <h6>  الـعـنـوان :  </h6>
                    <h6>  <?php echo $row['OrderAddress']; ?>  </h6>
                </div>
                <hr class="top">
                <div class="main-strip">
                    <h6>الـوصـف</h6>
                ...
                <div class="shipping-total-2">
                    <h6>مبلغ التوصيل</h6>
                    <h6 style="font-weight: bold;"><?php echo $row['OrderDeliveryCost']; ?></h6>
                </div> 
                <div class="shipping-total-2">
                    <h6>الـمـبـلـغ الـكـلـي</h6>
                    <h6 style="font-weight: bold;"><?php echo ($row['OrderCost'] + $row['OrderDeliveryCost']); ?></h6>
                </div>
            <?php endif; ?>
        </div>
        ...
        function AfterPrint(OrderV) {
            if (OrderV == 1) {
                window.location.replace("http://localhost/Glomus/BarPrintOrder.php");
            } else if (OrderV == 0) {
                window.location.replace("http://localhost/Glomus/KitchenPrintOrder.php");
            }
        }
        </script>
    </body>
